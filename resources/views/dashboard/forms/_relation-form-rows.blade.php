@php($itemsEmpty = is_array($options['items']) ? empty($options['items']) : $options['items']->isNotEmpty())
@php($items = $itemsEmpty ? $options['items'] : collect(array_values(old(Str::plural(Str::camel($options['resource']))) ?? [])))
