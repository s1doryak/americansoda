@php($itemsNotEmpty = isset($options['items']) ? (is_array($options['items']) ? !empty($options['items']) : $options['items']->isNotEmpty()) : false)
...
@if ($items)
    @foreach($items as $idx => $item)
        ...
        @include('dashboard::resources.ltp_transfer.items._relation-form-row', [
            'idx' => $idx + 1,
            'item' => $item,
            'is_template' => false,
            'multiple_rows' => true,
