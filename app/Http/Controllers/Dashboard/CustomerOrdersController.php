...
class CustomerOrdersController extends ResourceController
{
		...
		return function ($customerOrder) {
			return function ($query) use ($customerOrder) {
				if (is_object($customerOrder)) {
					...
					$customerOrder->lazyLoad('customer');
					...
					)->where('customer_pricing_policies.customer_id', '=', $customerOrder->customer->id)
	...
	protected function getRedirectUrl($action, $customerOrder = null)
	{
		if ($customerOrder) {
			if (prefix_name()) {
				return route(sprintf('%s.%s.edit', prefix_name(), resource_name()), ['id' => $customerOrder->getKey()]);
			} else {
				return route(sprintf('%s.edit', resource_name()), ['id' => $customerOrder->getKey()]);
			}
		} else {
